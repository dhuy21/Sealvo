...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function startBackgroundLetterAnimations() {
    ...
    ); // Every 1.5-3.5 seconds (more frequent)
  ...
  function displayGrid() {
    ...
    if (isTouchDevice) {
      wordSearchGrid.addEventListener(
        'touchmove',
        function (e) {
          if (isSelecting) {
            e.preventDefault();
  ...
  function handleMouseOver(event) {
    ...
    const isValidDirection = directions.some(
      ([dx, dy]) =>
        dx === Math.sign(rowDiff) &&
        dy === Math.sign(colDiff) &&
    ...
    if (selectedCells.length > 1) {
      ...
      const isInLine =
        (dirX === 0 && newDirX === 0) ||
        (dirY === 0 && newDirY === 0) ||
        Math.abs(row - firstCell.row) === Math.abs(col - firstCell.col);
      if (!isInLine) return;
    }
