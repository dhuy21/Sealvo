...
class User {
  ...
  async findById(user_id) {
    try {
      this.checkDbConnection();
      const [rows] = await global.dbConnection.execute('SELECT * FROM users WHERE id = ?', [
        user_id,
      ]);
      return rows[0] || null;
    } catch (error) {
  ...
  async updateResetPasswordToken(email, token, expiresAt) {
    try {
      ...
      console.error(
        'Erreur lors de la mise à jour du jeton de réinitialisation du mot de passe :',
        error
      );
      throw error;
    }
  ...
  async getDateUpdatedStreak(id) {
    try {
      ...
      console.error(
        'Erreur lors de la récupération de la date de mise à jour de la série :',
        error
      );
      throw error;
    }
