...
class Learning {
  ...
  async getNumWordsByLevelAllPackages(user_id, level) {
    try {
      ...
      console.error(
        'Erreur lors de la récupération du nombre de mots par niveau dans tous les packages :',
        error
      );
      throw error;
    }
  ...
  async findWordsTodayToLearnAllPackages(user_id) {
    try {
      ...
      console.error(
        "Erreur lors de la récupération des mots à réviser aujourd'hui dans tous les packages :",
        error
      );
      throw error;
    }
  ...
  async findWordsTodayByLevel(package_id, level) {
    try {
      ...
      console.error(
        "Erreur lors de la récupération des mots à réviser aujourd'hui par niveau :",
        error
      );
      throw error;
    }
  ...
  async getRandomUserWords(package_id, limit = 5, levelGame) {
    try {
      ...
      const [words] = await global.dbConnection.execute(query, [
        package_id,
        levelGame,
        intervalDate,
      ]);
